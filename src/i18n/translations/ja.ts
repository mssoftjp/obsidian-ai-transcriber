...
const ja: TranslationKeys = {
	...
	settings: {
		...
		model: {
			...
			gptTranscribeDesc: '録音済み音声向けの推奨モデル',
			gptTranscribeDefault: 'GPT Transcribe（推奨・既定）',
			whisper: 'Whisper-1',
			whisperDesc: 'タイムスタンプが必要な場合',
			gpt4o: 'GPT-4o Transcribe',
			gpt4oDesc: '既存の高精度モデル',
			gpt4oMini: 'GPT-4o Mini Transcribe',
			gpt4oMiniDesc: 'GPT系で最も低コスト',
	...
	notices: {
		...
		vadDisabledFallback: 'ローカルVADモジュールを利用できないため、無音を除去せずに処理を続行します。',
	...
	modal: {
		...
		transcription: {
			...
			additionalProcessingCostNote: 'この金額は文字起こし分のみです。AI後処理では追加のAPI呼び出しを行うため、推定コストを上回る場合があります。',
			timeRange: '時間範囲',
