...
const zh: TranslationKeys = {
	...
	settings: {
		...
		model: {
			...
			gptTranscribeDesc: '最新高精度模型（推荐）',
			gptTranscribeDefault: 'GPT Transcribe（推荐）',
			whisper: 'Whisper-1',
			whisperDesc: '需要时间戳时使用',
			gpt4o: 'GPT-4o Transcribe',
			gpt4oDesc: '现有的高精度模型',
			gpt4oMini: 'GPT-4o Mini Transcribe',
			gpt4oMiniDesc: '成本最低的 GPT 转录选项',
			whisperNoTimestamp: 'Whisper-1（无时间戳）',
			whisperWithTimestamp: 'Whisper-1（带时间戳）',
	...
	modal: {
		...
		transcription: {
			...
			costNote: '基于文件大小的估算',
			costDetails: '详细说明',
			additionalProcessingCostNote: '此金额仅包含转录。AI后处理会额外调用API，最终费用可能高于该估算。',
			timeRange: '时间范围（可选）',
	...
	errors: {
		...
		notices: {
			...
			supportedFormats: '支持的格式：音频（MP3、WAV、M4A、FLAC、AAC、OGG、WMA Standard）、视频（MP4、M4V、MOV、AVI、MKV、WebM）',
			networkRetry: '检查网络连接后再试',
			formatConversion: '请转换为 WAV 或 MP3 格式后再试',
			sizeLimit: 'OpenAI 转录 API 每次上传的文件上限为 25MB。较大的媒体文件会分割为较小的片段处理。',
