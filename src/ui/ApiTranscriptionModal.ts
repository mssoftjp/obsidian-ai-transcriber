import { Modal, Notice, TFolder, Platform, Setting, getLanguage, ButtonComponent, normalizePath, type App, type Component, type TFile, type TextComponent } from 'obsidian';
...
export class APITranscriptionModal extends Modal {
		...
		this.logger.debug('Generated filename', { fileName });
			...
			default:
				formattedTranscription = `\n## AI Transcription - ${this.audioFile.name}\n*Generated by ${providerName} on ${timestamp}*\n\n${transcription}\n\n`;
					...
					// Open the created file for convenience (ignore failures; content is already written)
					try {
						const leaf = this.app.workspace.getLeaf(false);
					await leaf.openFile(createdFile);
					this.logger.info('File created and opened', { filePath, viewType: leaf.view.getViewType() });
				} catch (openError: unknown) {
					const err = openError instanceof Error ? openError : new Error(this.formatUnknownError(openError));
				...
				this.logger.error('Failed to write transcription to file', writeError);
					...
					try {
						...
						await this.app.vault.process(targetFile, (currentContent) => currentContent + '\n\n' + formattedTranscription);
						this.logger.trace('Fallback: Appended transcription to file');

						// Keep behavior consistent with the success path: try to open the created file
						try {
						const leaf = this.app.workspace.getLeaf(false);
						await leaf.openFile(createdFile);
						this.logger.info('File opened after fallback write', { filePath, viewType: leaf.view.getViewType() });
					} catch (openError: unknown) {
						const err = openError instanceof Error ? openError : new Error(this.formatUnknownError(openError));
						this.logger.warn('Failed to open created file after fallback write', { error: err.message, filePath });
					}
	...
	private async ensureFolderPath(folderPath: string): Promise<void> {
		...
		if (existing) {
			...
			throw new Error(t('errors.createFileFailed', { error: `${normalizedPath} is not a folder` }));
		}

		await this.app.vault.createFolder(normalizedPath);
		this.logger.trace('Created folder', { path: normalizedPath });
	}
	...
	override onClose() {
		...
		this.parentComponent.removeChild(this.loadingAnimation);
