...
export class APITranscriptionModal extends Modal {
	...
	private async insertTranscription(transcription: string, modelUsed?: string) {
		...
		const requestedPath = normalizePath(outputFolder ? `${outputFolder}/${fileName}` : fileName);
		...
		case 'callout':
			...
			break;
		case 'quote':
			formattedTranscription = `\n> **AI Transcription - ${this.audioFile.name}**\n> *Generated by ${providerName} on ${localTimestamp}*\n>\n${transcription.split('\n').map(line => `> ${line}`).join('\n')}\n\n`;
			break;
		default:
			formattedTranscription = `\n## AI Transcription - ${this.audioFile.name}\n*Generated by ${providerName} on ${localTimestamp}*\n\n${transcription}\n\n`;
		}
		...
		try {
			...
			this.logger.warn('Transcription note was saved, but a workspace event failed', {
				filePath,
				error: eventError instanceof Error ? eventError.message : this.formatUnknownError(eventError)
			});
		}
